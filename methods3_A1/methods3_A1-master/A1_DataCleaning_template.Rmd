...
```{r}
#first way to do it
demo_sub <- subset(demo, select = c(SUBJ, VISIT, Ethnicity, Diagnosis, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization))

#second way to do it
token_sub <- select(token, SUBJ, VISIT, types_MOT, types_CHI, tokens_MOT, tokens_CHI )

LU_sub <- select(LU, SUBJ, VISIT, CHI_MLU, MOT_MLU)
```
...
```{r}
data1 <- merge(LU_sub, demo_sub, all = TRUE)
data <- merge(data1, token_sub, all = TRUE)
```
...
```{r}
#Create a new dataframe with just visit 1 (and disregarding 'visist')
visitdf <- subset(data, VISIT == 1)
clinvari <- select(visitdf, SUBJ, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)

#renaming ADOS, ADOS1
colnames(clinvari)[2] <- "ADOS1"
colnames(clinvari)[3] <- "MullenRaw1"
colnames(clinvari)[4] <- "ExpressiveLangRaw1"
colnames(clinvari)[5] <- "Socialization1"

#merging with old dataframe
final <- merge(clinvari, data, by = "SUBJ")
```
...
```{r}
#renamin variables for clarity
final <- rename(final, sev_aut_sympt = ADOS) #ADOS changed to sev_aut_sympt (severity of autistic symptoms)
final <- rename(final, sev_aut_sympt_1 = ADOS1)

final <- rename(final, non_verbal_IQ = MullenRaw) 
final <- rename(final, non_verbal_IQ_1 = MullenRaw1)

final <- rename(final, verbal_IQ = ExpressiveLangRaw) 
final <- rename(final, verbal_IQ_1 = ExpressiveLangRaw1)

final <- rename(final, unique_words_MOT = types_MOT) 
final <- rename(final, unique_words_CHI = types_CHI)

final <- rename(final, overall_words_MOT = tokens_MOT)
final <- rename(final, overall_words_CHI = tokens_CHI)

final <- rename(final, social_int_skills = Socialization) #Socialization indicates social interaction skills and social responsiveness
final <- rename(final, social_int_skills_1 = Socialization1)


#anonymizing subjects
final$SUBJ <- as.factor(final$SUBJ)
final$SUBJ <- fct_anon(final$SUBJ, prefix = "")

#changing gender levels
final$Gender <- as.factor(final$Gender)
final$Gender <- fct_recode(final$Gender, M = "1", F = "2")

#changing values of diagnosis
final$Diagnosis <- as.factor(final$Diagnosis)
final$Diagnosis <- fct_recode(final$Diagnosis, ASD = "A", TD = "B")

write.csv(final, "df_portfolio1.csv")
```
